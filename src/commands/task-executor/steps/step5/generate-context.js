...
const generateContextFile = async (task) => {
  ...
  const taskMd = fs.existsSync(folder('TASK.md')) ? fs.readFileSync(folder('TASK.md'), 'utf8') : '';
  ...
  let researchSummary = 'No research file available';
  if (fs.existsSync(folder('RESEARCH.md'))) {
    ...
    if (strategyMatch) {
      researchSummary = strategyMatch[1].trim().split('\n').slice(0, 5).join('\n');
    }
  }
